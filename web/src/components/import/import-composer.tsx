  ...
  return (
    <div ref={pageRef} className="mx-auto max-w-[1180px]">
      <section className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_390px] lg:items-end">
        <div>
          <p data-intro className="type-eyebrow flex items-center gap-2 text-primary"><MusicNotesIcon className="size-4" weight="fill" /> From Instagram to playlist</p>
          <h2 data-intro className="type-display mt-4 max-w-3xl">Don't leave that song<br /><span className="text-[#a7a7a7]">behind.</span></h2>
          <p data-intro className="type-body mt-5 max-w-2xl">Add your Story and Highlight links. We'll find the music; you choose the right Spotify match.</p>
        </div>
        <div data-intro className="hidden border-l border-white/10 pl-7 lg:block">
          <p className="text-xs font-extrabold text-white">You stay in control at every step</p>
          <p className="mt-2 text-xs leading-5 text-muted-foreground">No track is added to your playlist without your approval.</p>
        </div>
      </section>
        ...
        <div className="grid xl:grid-cols-[minmax(0,1fr)_330px]">
          <section className="min-w-0 p-5 sm:p-7 lg:p-8">
            <div className="flex items-start justify-between gap-5">
              <div>
                <p className="type-eyebrow text-muted-foreground">01 · Source</p>
                <h3 className="type-section-title mt-2">Add your links</h3>
                <p className="mt-1.5 text-xs leading-5 text-muted-foreground">One Instagram link or Highlight ID per line.</p>
              </div>
            ...
            <div className="mt-6 overflow-hidden rounded-xl border border-white/10 bg-[#0f0f0f] transition focus-within:border-white/25 focus-within:ring-2 focus-within:ring-white/[0.04]">
              <div className="flex min-w-0 items-center justify-between gap-2 border-b border-white/[0.07] px-3 py-2.5 sm:px-4">
                ...
                <div className="flex shrink-0 items-center gap-1">
                  {sourceText && <button type="button" className="rounded-full px-2.5 py-1.5 text-[11px] font-bold text-muted-foreground transition hover:bg-white/[0.06] hover:text-white" onClick={() => setSourceText("")}>Clear</button>}
                  <button type="button" className="rounded-full bg-white px-3 py-1.5 text-[11px] font-extrabold text-black transition hover:scale-[1.03] active:scale-95" onClick={pasteFromClipboard}><span className="hidden sm:inline">Paste from clipboard</span><span className="sm:hidden">Paste</span></button>
                </div>
              </div>
              <Textarea
                value={sourceText}
                ...
                aria-label="Instagram Story and Highlight links"
              />
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-2">
              <div className="rounded-xl border border-white/[0.07] bg-[#141414] p-4">
                <div className="mb-3 flex items-center justify-between gap-3">
                  <span className="flex items-center gap-2 text-xs font-extrabold"><LaptopIcon className="size-4 text-muted-foreground" weight="duotone" /> Instagram session</span>
                  <span className="text-[10px] font-bold text-muted-foreground">For private content</span>
                </div>
                <Select value={browserSource || config.data?.defaultBrowserSource || "none"} onValueChange={changeBrowserSource}>
                  ...
                  <SelectContent><SelectItem value="none">No login required</SelectItem><SelectItem value="firefox">Firefox</SelectItem><SelectItem value="brave">Brave</SelectItem><SelectItem value="chrome">Google Chrome</SelectItem><SelectItem value="chromium">Chromium</SelectItem><SelectItem value="edge">Microsoft Edge</SelectItem><SelectItem value="safari">Safari</SelectItem></SelectContent>
                </Select>
                <p className="mt-2.5 flex items-center gap-1.5 text-[10px] text-muted-foreground"><LockKeyIcon className="size-3.5" weight="duotone" /> Your cookies never leave this device.</p>
              </div>
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <span className="flex items-center gap-2 text-xs font-extrabold"><FileCodeIcon className="size-4 text-muted-foreground" weight="duotone" /> HTML backup</span>
                  <span className="text-[10px] font-bold text-muted-foreground">Optional</span>
                </div>
                <button type="button" className="mt-3 flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-white/[0.06] text-xs font-extrabold transition hover:bg-white/[0.1]" onClick={() => fileInput.current?.click()}><CloudArrowUpIcon className="size-4" weight="duotone" /> Choose or drag a file</button>
                <p className="mt-2.5 text-[10px] text-muted-foreground">HTML files from your Instagram data export.</p>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {files.map((file, index) => (
                  <span key={`${file.name}-${index}`} className="flex max-w-full items-center gap-2 rounded-full bg-white/[0.07] py-1.5 pl-3 pr-2 text-xs font-semibold">
                    ...
                    <button type="button" className="rounded-full p-0.5 text-muted-foreground hover:bg-white/10 hover:text-white" onClick={() => setFiles((current) => current.filter((_, fileIndex) => fileIndex !== index))} aria-label={`Remove file ${file.name}`}><XIcon className="size-3.5" weight="bold" /></button>
                  </span>
                ))}
              </div>
            )}
          ...
          <aside className="min-w-0 border-t border-white/[0.07] bg-[#111111] p-5 sm:p-7 xl:flex xl:flex-col xl:border-l xl:border-t-0">
            <p className="type-eyebrow text-muted-foreground">02 · Destination</p>
            <h3 className="type-section-title mt-2">Spotify connection</h3>

            <div className="mt-5">
              {spotify.isPending ? (
                <div className="flex items-center gap-3 rounded-xl border border-white/[0.07] bg-[#181818] p-4"><CircleNotchIcon className="size-4 animate-spin text-muted-foreground" weight="bold" /><div><p className="text-xs font-extrabold">Checking your account</p><p className="mt-1 text-[10px] text-muted-foreground">Restoring saved session</p></div></div>
              ) : spotify.isError ? (
                <button type="button" onClick={() => spotify.refetch()} className="flex w-full items-center gap-3 rounded-xl border border-white/[0.07] bg-[#181818] p-4 text-left transition hover:bg-[#242424]"><ArrowClockwiseIcon className="size-4" weight="bold" /><div><p className="text-xs font-extrabold">Check again</p><p className="mt-1 text-[10px] text-muted-foreground">Could not verify the current connection.</p></div></button>
                ...
                <div className="flex items-center gap-3 rounded-xl border border-[#1ed760]/15 bg-[#1ed760]/[0.055] p-4">
                  ...
                  <div className="min-w-0 flex-1"><p className="truncate text-xs font-extrabold">{spotify.data.profile.displayName}</p><p className="mt-1 flex items-center gap-1.5 text-[10px] font-bold text-[#1ed760]"><CheckCircleIcon className="size-3" weight="fill" /> Ready for playlist creation</p></div>
                ...
                <div className="rounded-xl border border-white/[0.07] bg-black p-4">
                  ...
                  <p className="mt-3 text-xs font-extrabold">Connect to create a playlist</p>
                  <p className="mt-1.5 text-[10px] leading-4 text-muted-foreground">You can scan music without connecting, and connect your account later on the results screen.</p>
                  <SpotifyConnectButton className="mt-4 w-full" size="sm" returnPath="/app">Connect Spotify</SpotifyConnectButton>
                </div>
              )}
            </div>
            ...
            <div className="flex items-end justify-between">
              <div><p className="text-[10px] font-bold text-muted-foreground">Sources to process</p><p className="mt-1 text-3xl font-extrabold tracking-[-0.05em]">{sourceCount}</p></div>
              <ImagesIcon className="size-7 text-[#5a5a5a]" weight="duotone" />
            </div>
            <p className="mt-3 text-[11px] leading-5 text-muted-foreground">We'll scan the sources first. You can change the matches we find on the next screen.</p>
            ...
            <div className="mt-auto pt-7">
              <Button size="lg" type="submit" className="w-full justify-between px-5" disabled={startImport.isPending || sourceCount === 0 || sourceCount > 12}>
                <span className="flex items-center gap-2">{startImport.isPending ? <CircleNotchIcon className="animate-spin" weight="bold" /> : <MagnifyingGlassIcon weight="bold" />}{startImport.isPending ? "Scanning…" : "Scan music"}</span><ArrowRightIcon weight="bold" />
              </Button>
              <p className="mt-3 text-center text-[10px] font-semibold text-muted-foreground">Usually takes less than a minute</p>
            </div>
          </aside>
        </div>
      </form>
      ...
      {config.data && !config.data.ytDlpAvailable && <div className="mt-6 rounded-xl border border-amber-300/15 bg-amber-300/[0.04] px-4 py-3 text-xs leading-5 text-amber-200"><code className="font-bold">yt-dlp</code> was not found for local Instagram sessions. Public content or HTML upload can be used instead.</div>}
    </div>
  )
}